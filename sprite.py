...
class SpriteAnimation(pygame.sprite.Sprite):
    def __init__(self, sp: str, i2: int, anc: int, alt: int, fps: float):
        super().__init__()
        # toma la ruta de la imagen hasta el nombre de la imagen
        # sin contar su extencion (obligatoriamente tiene que ser png)
        self.sp = sp
        ...
        # es la pausa por cada frame
        # que va a tener para el cambio de cada imgaen
        self.fps = fps

        # variable para rastrear si la animación ha terminado
    ...
    def cargadesrpite(self):
        ...
        spriteArray = [
            pygame.image.load(f'{self.sp}{i}.png').convert_alpha()
            for i in range(self.i2)
            ]
        # lo pasamos a otra lista para cambiar su escalado
        sprite_escalado = [
            pygame.transform.scale(spriteArray[i], (self.anc, self.alt))
            for i in range(self.i2)
            ]
    ...
    def animation(self, screen, posx, posy, flipx):
        ...
        SpriteFlip = [
            pygame.transform.flip(sprite, flipx, False)
    ...
    def animation_2(
            self, screen: str, posx: int, posy: int, flipx: bool,
            background: str
            ):
        ...
        for i in range(self.i2):
            ...
            sprite_rect = pygame.Rect(
                posx, posy, SpriteFlip[i].get_width(),
                SpriteFlip[i].get_height()
                )
